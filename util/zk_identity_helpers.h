...
void ExportProof(LibsnarkAuthenticationData &auth, std::string fname){
  AuthenticationProof proof = ExtractAuthenticationProof(auth.proof);
  ...
  for(int i = 0; i < auth.primary_input.size(); i++){
    f << "\tinput[" << i <<"] = 0x" << HexStringFromLibsnarkBigint(auth.primary_input[i].as_bigint()) << ";" << std::endl;
  }
